...
if license_choice == "MIT":
    ...
    license_spdx_id = "UNLICENSED" # Common for package.json
    license_badge = "" # No badge for proprietary
    if os.path.exists(mit_license_file):
        os.remove(mit_license_file)
    if os.path.exists(apache_license_file):
        os.remove(apache_license_file)
    with open(final_license_file, "w", encoding='utf-8') as f:
...
if os.path.exists(package_json_path):
    ...
    package_data["license"] = license_spdx_id # Use SPDX ID or "UNLICENSED"
    
    with open(package_json_path, "w", encoding='utf-8') as f:
        json.dump(package_data, f, indent=2) # indent=2 for pretty printing
        f.write("\\n") # Add trailing newline
    print(f"Updated package.json license to {license_spdx_id}")
